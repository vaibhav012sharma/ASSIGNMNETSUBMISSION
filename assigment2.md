...
* conserve memory usuageQ2   MUTUABLE AND IMMUTUABLE DATA TYPES 
...
A5  if i want to get uniques items(no duplicate items) and does not matter order so we can use set data structure  ,while list data structure is useful to add new data and remove data  also modify the data 
...
a.insert(4,{1,2,3})
a.extend(b)
print(a)Q7 How tuple  ensure  data integrity in python 

A7 data integity ensure the data is accurate and consistent 
...
*but list data structure can not represent the fixed datasets list are useful for sorting and managing collections Q13  How do sets handle duplicate values in Python?
Ans  Sets can not allow  dupicate values bcz set is unordered collections of unique items
...
print(4 in a)Q15  Can you modify the elements of a tuple? Explain why or why not?
A15  NO bcz tuple is immutuable so we can not add, remove and modify data in tuple (only read and access data by using index and slicing)Q16 NESTED DICTIONARY , GIVE EXAMPLE AND USE CASES
A16 Nested Dictionary - when we create dictionary inside dictionary this approach known as nested dictionary.
ex 
...
A17 O(1)Q18 In what situations are lists preferred over dictionaries?
A18  *for ordered collections of items  and sequencing operations like( append, insert,extend,pop,remove,del)
* list act as stack and queue 
Q20 differnce b/w list and dictionary in data retrieval 
ANS data retrieval  it is a process to finding ,copying or moving the information from the system
